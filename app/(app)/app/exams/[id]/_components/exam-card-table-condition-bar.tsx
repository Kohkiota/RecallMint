...
export function ConditionBar({
  ...
  // S2b-3: render-side partition by kind (deriveConditions 契約不変)
  const sortConditions = conditions.filter((c) => c.kind === 'sort')
  const filterConditions = conditions.filter((c) => c.kind === 'filter')
  ...
  // 列表示名解決ヘルパー
  const getDisplayName = (columnId: string): string => {
    const col = table.getColumn(columnId)
    const headerDef = col?.columnDef.header
    return typeof headerDef === 'string' ? headerDef : columnId
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 px-1 py-2 text-sm">
      ...
      {sortConditions.map((condition) => {
        const displayName = getDisplayName(condition.columnId)
        return (
          <span
          ...
          >
            {/* chip body: クリックで方向 flip。S2b-3: 「並び替え: 」プレフィックス削除 */}
            <button
              type="button"
              onClick={() => handleFlipSort(condition.columnId)}
              className="inline-flex items-center gap-0.5 hover:text-foreground"
            >
              {displayName} {condition.desc ? '↓' : '↑'}
            </button>
            {/* × ボタン: sort 解除 */}
            <button
              type="button"
      ...
      {/* 縦区切り: 両ゾーン非空時のみ */}
      {sortConditions.length > 0 && filterConditions.length > 0 && (
        <div data-testid="zone-separator" className="h-4 w-px bg-border" />
      )}

      {/* Filter zone (right) */}
      {filterConditions.flatMap((condition) => {
        const col = table.getColumn(condition.columnId)
        const displayName = getDisplayName(condition.columnId)

        // S2b-3 tags 特例: 選択 option ごとに個別 chip を展開。
        // 特例は columnId === 'tags' 分岐に局所化 — 他 filter / sort の generic 経路は不変。
        if (condition.columnId === 'tags') {
          const rawTagFilter = condition.value as TagFilterValue | undefined
          const allAssignedOptionIds = Object.values(rawTagFilter ?? {}).flat()

          return Object.entries(rawTagFilter ?? {}).flatMap(([categoryId, optionIds]) =>
            optionIds.map((optionId): React.JSX.Element => {
              const category = editorContext.categories.find((c) => c.id === categoryId)
              const option = editorContext.options.find((o) => o.id === optionId)
              const catName = category?.name ?? categoryId
              const optName = option?.name ?? optionId
              // 欠損 option は colorToClass(undefined) = COLOR_NULL_CLASS (無彩色 fallback)
              const chipColorClass = colorToClass(option?.color)
              const label = `${catName}: ${optName}`

              return (
                <span
                  key={`filter-tags-${optionId}`}
                  data-testid={`condition-chip-filter-tags-${optionId}`}
                  className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs ${chipColorClass}`}
                >
                  {/* CardTagAddPopover: chip body が trigger。selectOnly で作成/編集非表示。 */}
                  <CardTagAddPopover
                  ...
                  />
                  {/* × ボタン: その option のみ除去。stopPropagation で CardTagAddPopover trigger と分離。
                      除去経路 = 既存 handleTagsChipToggle (空カテゴリ prune + 空 map → undefined)。 */}
                  <button
                    type="button"
                    aria-label={`フィルタを解除: ${label}`}
                    onClick={(e) => {
                      e.stopPropagation()
                      handleTagsChipToggle(categoryId, optionId)
                    }}
                    className="text-muted-foreground hover:text-foreground"
                  >
                    ×
      ...
      {/* クリア: 条件が 1 件以上ある時のみ表示 (conditions.length > 0 は null check 済) */}
      <button
        type="button"
