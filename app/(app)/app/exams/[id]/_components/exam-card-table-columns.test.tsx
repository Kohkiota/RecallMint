...
describe('T3: column sizing', () => {
  ...
  it('各列の size が仕様値と一致する', () => {
    const sizeMap: Record<string, number> = {
      select: 88, // 行操作 button 常時表示化(iPad hover 不能環境対応)で checkbox 隣接に移設 + Grid-3 §7.2 の行メニュー trigger 追加で再拡幅。row-dnd task-3 で drag handle を配線したが provider 未配線 (task-4 まで) は null 描画のため幅は不変
