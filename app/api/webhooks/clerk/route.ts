...
import { users, clerkEvents, deletionFailures } from '@/lib/db/schema'
...
async function handleUserDeleted(userId: string): Promise<void> {
  ...
  // Spec §8.1 layer 1: DB deletedAt set + users.id / stripeCustomerId returning。
  // F-5 で users.id (内部 UUID) も returning に追加: deletion_failures.user_id が
  // uuid 化したため、recordFailure に internal UUID を渡す必要がある。
  // 失敗は throw → outer catch → notifyOps 経路 (delete_failures に書かない、
  // sub_id 概念がないため)。
  ...
  const internalUserId = updated[0]?.id
  const customerId = updated[0]?.stripeCustomerId
  ...
  if (!customerId) return // 課金連携なしの user (Free プラン削除等) → 終了

  // Spec §8.1 layer 2 / §8.4 / §8.5: Stripe sub list + cancel ループ
...
/**
 * Spec §8.1: deletion_failures 書き込み + notifyOps の合成。
...
async function recordFailure(args: {
  ...
  clerkUserId: string
  subId: string | null
  kind: 'list' | 'cancel' | 'customer_missing'
  errorMessage: string
}): Promise<void> {
  const db = getDb()
  ...
  await notifyOps('stripe sub cancel failure during deletion', {
    userId: args.internalUserId,
    clerkId: args.clerkUserId,
    subId: args.subId,
    kind: args.kind,
