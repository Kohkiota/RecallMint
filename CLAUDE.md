...
**全 sprint 共通(audit gate)**: 完了時に `pnpm run audit`(= 規律外 key 検査 `scripts/check-audit-config.mjs` + `pnpm audit --audit-level high`)exit 0。報告 chat に「pnpm run audit exit 0 確認済」を1行明記。閾値は **high**(二段階制は 2026-07-21 の transitive bump sprint 完了で解消済・経緯 = `docs/audit/dependency-audit-ledger.md`)。規律: ① moderate 以下の検出は gate 対象外・台帳へ follow-up 記録 ② high/critical の受容は `pnpm-workspace.yaml` の `auditConfig.ignoreGhsas` への明示追加のみで行い、追加時は GHSA/CVE・理由・再検討条件を台帳にセットで記録(allow-list は patched 不在 or 真の受容限定)。CLI `--ignore` は使用禁止(workspace yaml へ `ignoreCves` を無断書込する別動作)— `auditConfig` の規律外 key は check-audit-config.mjs が機械検出して gate を落とす(検出時 = 無許可 suppression として revert・台帳「運用」参照)。注意: builtin `pnpm audit` は script より優先され level 未指定(low)で走るため、gate は必ず `pnpm run audit`。
