...
5. **矩形へのパディング加算**(box2d 目視 go・2026-07-28 追記): box_2d は図にぴったり張り付き、図下の出所表記(「出所: 財務省ホームページ」等)や軸ラベルが枠外に落ちる。切り出し側で**決定的に**上下左右へ固定割合(3〜5% 目安・実装時に現物調整)を加算し画像端でクランプする。**モデルに「余裕を持って囲め」と指示する形は採らない**(確率的で検証不能)。余白過多の実害は小さく、切れて情報が失われる方が回復不能ゆえ非対称に広めへ倒す。
6. **出所・キャプションは切り出し範囲に含める**方針とする(項目5 と一体)。
7. **図中文字が本文テキストにも重複抽出されるのを許容**する(本文側から図内文字を除去する判定は入れない — 誤判定で本文欠落するリスクの方が重い)。
