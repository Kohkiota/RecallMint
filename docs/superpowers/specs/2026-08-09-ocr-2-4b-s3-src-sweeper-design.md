...
- **override は production で 400**(A1・OT 裁定 2026-08-09): `VERCEL_ENV === 'production'` で `?cutoffMinutes=` が付いていたら受理しない。「手動 GET 限定・リスクは stg に閉じる」を運用の約束でなく機械強制にする(secret を持つ者が prod の保持 policy を 6h → 15min に変えられる操作を構造的に塞ぐ)。prod の手動 GET は既定 6h でのみ走る。
- **key 形の関門**: 候補は `^src/{uuid}/{uuid}/{uuid}\.pdf$` に一致するものだけ DELETE 対象にする(§2 の破壊境界二重関門と同旨)。**uuid の照合は case-insensitive**(A6・OT 裁定): `z.uuid` は大文字 hex を通すため、client 供給の `uploadSessionId` / `fileId` 由来で大文字を含む正規 key が存在しうる。小文字限定にするとその object を恒久 skip して mismatch 行を毎日吐き続ける。削除対象は広がるが「`src/` prefix + uuid 形 3 セグメント + age 超過」の三重条件内。不一致 object は**削除せず** `reason: 'pattern_mismatch'` で記録して lifecycle に委ねる(§10 論点 3)。
...
| `r2_sweep_overdue` | r2 | src_sweep.overdue | src_sweep | state_mismatch | §3.6 の期限超過 alert・1 run 最大 1 行。context = `{ count, oldestKey, oldestAgeHours, partial }`(`partial: true` = listing 上限で全域を見ていない) |
...
- 各記帳は §2 の `recordSrcPurgeRow` と同じ個別 try/catch で包む(notifyOps throw の伝播で以降の削除を巻き込まない)。**記帳自体の失敗は `logger.error` + summary の `recordErrors` に必ず反映**する — alert 経路が壊れたことが silent にならないため(唯一の観測点が記帳である以上、その失敗は別経路で見えなければならない)。
- **行 quota は種別ごとに独立**(A2・OT 裁定 2026-08-09): 実削除失敗 ≤20 / pattern 不一致 ≤5 / overdue ≤1 / incomplete ≤1。overdue と incomplete は**枠外**(構造的に 1 run 1 行で bounded)。種別を分けたのは、mismatch の洪水が実削除失敗と overdue を抑圧しないため(共有 20 枠だと辞書順先頭の mismatch だけで枠が尽きる)。上限で落ちた行数は incomplete の `suppressedFailures` に載せる。**§2 の heldFailure(最後の 1 枠を incomplete に譲る)は sweeper では採らない** — quota 分離で不要になったため。§2 側の実装は変更しない(動いているものを一貫性のために触らない)。
...
- 72h の根拠: sweeper の保持上限(§4 の式・条件付き worst ≈ 55h)と lifecycle 実効上限(≈48h)の両方を超える最小の丸い値。ここに到達した object は「どの機構も回収しなかった」ことの証拠。
- **適用範囲(A3・OT 裁定 2026-08-09)**: この alert が観測するのは **listing 上限 `SWEEP_MAX_LIST_PAGES`(10 page ≈ 10,000 key)の範囲内**であって `src/` 全域ではない。R2 listing は key 順で age 順ではないため、11 page 目以降の 72h 超 object は検知できない。したがって本 alert は「lifecycle 効果監視の常設 readback(fact-finding §3.3 の運用形)」であるが、その保証は **bounded partial observation** に限る。`truncated` の run では context に `partial: true` を入れ、「全域を見た上で 0 件」と「見た範囲で 0 件」を混同できない形にする。全域保証が要るなら別途 inventory 走査が要る(本 spec の scope 外)。
...
保持時間 T = cutoff + 実行間隔 × 2(1 回 skip 許容)+ schedule 揺らぎ + 走査最大時間:

- Pro plan: 6h + 48h + ~1min + 300s ≈ **54.2h**
- Hobby plan: 6h + 48h + 59min + 300s ≈ **55.2h**
- 正常系(skip なし): 6h + 24h + 揺らぎ ≈ **30h**

**これは不変条件ではなく条件付き SLO**(A4・OT 裁定 2026-08-09)。成立の前提は ① cron が毎日発火する ② 対象が listing 上限内で走査される ③ live-op skip / DB 判定失敗が連続しない(skip ≤1 回)の 3 つで、いずれかが破れれば 55h を超えて残りうる(cron 停止・先頭 page の starvation・連日 live・DB 連続障害・日次能力超過)。**前提破れの検知は overdue alert(§3.6・72h)と incomplete 行(§3.5・phase)が担う**: 55h は「正常時に期待してよい値」であり、それを超えた事実は alert で loud になる。この 2 文を分離せずに書く(数値だけが独り歩きすると hard upper bound と誤読される)。

ALERT_AGE 72h > 上記 T(両 plan)であり、alert は「前提が成立している範囲の正常な遅延」では発火しない。**前提 readback(plan 段階・OT)**: ① Vercel plan の確定(maxDuration 720 が既に prod で動いている事実は Pro を示唆するが、記録の不在を事実にしない — 要確認)② stg deployment の形態(stg が別 project の production なら cron は stg でも発火する — §8 の env gating に影響)。
...
2. **DELETE 対象は `^src/{uuid}/{uuid}/{uuid}\.pdf$`(case-insensitive)一致かつ age > cutoff のみ**(二重関門・不一致は記録のみ)
3. **user の DELETE batch 直前に live-op 除外を評価**し、live または判定不能なら skip(fail-safe)
4. **overdue 評価は DELETE 実行前の snapshot に対して行う**(閾値は cutoff から独立した 72h 固定)
5. **lane は throw しない**(内部 catch → summary / 台帳。runner 外周 catch は 500)
6. **記帳 1 本ごとに個別 try/catch**、かつ記帳失敗は `logger.error` + summary `recordErrors` に反映(silent 劣化禁止)
7. **CRON_SECRET が空なら必ず 401**(preview/local の '' fallback を認証成立にしない)。auth は query 検証より先に評価する
8. **production では `?cutoffMinutes=` を受理しない**(400・A1)
9. **台帳行 quota は種別独立**(実削除失敗 ≤20 / mismatch ≤5 / overdue ≤1 / incomplete ≤1・A2)
10. **既存 `listObjects` / `listObjectsBounded` の signature・挙動不変**(§1/§2 の regression pin 維持)
...
**A1 との相互作用(§4 readback ② に依存)**: override は `VERCEL_ENV === 'production'` で 400。stg が別 project の production deployment なら **stg でも override が使えない** — その場合の smoke は手順 2 を「6h 待ち」に置き換える(A1 を smoke の都合で緩めない)。stg が preview tier なら override が効く。どちらかは OT の readback で確定する。
...
2. cutoff 経過を待つ(stg が preview tier なら override `?cutoffMinutes=15` で 15min に短縮可・§3.2 / production tier なら 6h 待ち)
...
- parse: `<Contents>` block 単位の Key+LastModified 対応付け / LastModified 欠落・非 ISO page は失敗(不変条件 1)/ 既存 listObjects 系の挙動不変(既存 test green が pin)
- 選定(pure 関数に切り出す): cutoff 境界(6h ± ε)/ pattern 不一致の除外と記録(**大文字 uuid は一致 = 正例**・A6)/ overdue の閾値と「DELETE 前 snapshot」評価順 / user 順が oldest 昇順
- live-op 除外: live user の全候補 skip / 判定クエリ失敗 → skip + phase `live_check` / **除外評価が DELETE より前であることの呼び出し順 pin**
- 予算: 時刻注入(§2 の `now: () => number` idiom)で deadline 打ち切り / phase 優先順位統合
- 台帳: catalog tuple 追加(14→17・ユニーク性 test の件数更新)/ 記帳個別 try/catch(notifyOps throw で後続削除が止まらない + `recordErrors` に加算)/ **quota 各境界**(実失敗 20 / mismatch 5 / overdue 1 / incomplete 1・超過分は `suppressedFailures`)
- auth: 空 secret → 401 / Bearer 不一致 → 401 / production env 欠落 → throw(既存 gate の挙動確認)
- override: 下限未満・非整数 → 400 / **production tier + `cutoffMinutes` → 400**(A1)/ override 使用時 summary に `cutoffOverrideMinutes` が載る / **override で cutoff を縮めても overdue 閾値(72h)は不変**(OT 裁定条件の pin)
...
3. **pattern 不一致 = 削除しない(記録のみ・lifecycle 委ね)承認**。不一致 key は「未知の将来 lane か配置ミス」でありうるため無条件削除は二重関門の意義を自壊させる。lifecycle 不全時に残り続ける帰結は overdue alert が毎日検知する = operator に判断が上がる loud な形で正しい

### 10.1 Codex plan cross-check の裁定(OT・2026-08-09・確定。raw = `docs/codex/2026-08-09-plan-ocr-2-4b-s3-src-sweeper.md`)

採用 6 件(A1 production override 400 = §3.2/§7-8 / A2 quota 種別独立 = §3.5/§7-9 / A3 partial observation = §3.6 / A4 条件付き SLO = §4 / A5 §13 追記 2 件 / A6 uuid case-insensitive = §3.2/§7-2/§9)。dead-man 監視は今 sprint 受容 = §13。

不採用(記録のみ): mismatch の 4 軸別 entry 化(§2 の `reason` discriminator が OT 裁定済 precedent・集計は reason 必須で読む)/ pagination cursor(「sweep 状態を永続化しない」に抵触・帰結は §13 で受容)/ 負荷テスト・`vercel.json` 構造テスト(YAGNI・review で見る)/ bulk live 判定(逐次 tx は候補 user 数ぶんで bounded・RLS 境界を複雑化しない)/ 207 multi-status(lane 1 本で YAGNI)。
...
- **成功 run の永続記録なし / dead-man 監視は今 sprint 対象外**(OT 裁定 2026-08-09): 実行履歴は Vercel logs の保持期間に依存し、cron 停止・CRON_SECRET 欠落・route 障害は「alert 自体が実行されない」形で沈黙する。backstop = lifecycle(効果は §4 判定待ち)+ OT の随時手動 GET + prod 反映後チェックリストへの「Vercel dashboard の cron 実行履歴を随時確認」の追加。恒久監視(外形監視 / dead-man switch)は asset reconciler lane 追加 sprint で再訪する。scheduler の実発火検証は prod 反映後の Vercel dashboard(OT)で、close 条件外(kickoff)
- **先頭 page の占有(starvation)は受容**(A5(a)): listing は毎回 key 順の先頭から最大 10 page。pattern 不一致 object は削除しないため、辞書順先頭に 10,000 件超の不一致 object が滞留すると後続 page へ到達できない。pagination cursor を持てば解けるが「sweep 状態を永続化しない」(§3.7・台帳なし確定)と抵触するため採らない。この状態は overdue alert が **毎日発火し続ける** loud な形で表面化し、operator の判断(手動 script / lane 追加)に上がる — silent に壊れない
- **same-key 再 PUT との競合は構造的に不成立**(A5(b)): listing で候補化した key が DELETE までの間に再 PUT され「新しい object を消す」病理は、presign の有効期限 600s < override 下限 900s(および既定 cutoff 6h)により正規 client では起こらない — 候補化された時点でその key の presign は既に失効しており、同一 key への新規 PUT 経路が無い(`uploadSessionId` / `fileId` は upload ごとに新規採番)。したがって **DELETE 前 HEAD は採らない**(HEAD 後 overwrite の TOCTOU が残るうえ候補数ぶんの API コストが増える)。15min という override 下限はこの不成立性の根拠でもある
