...
- **公開文言(3 面共通・具体的な待ち時間は書かない)**: 「処理が中断された可能性があります。しばらく待ってから再度お試しください。処理状況は試験一覧で確認できます。」相当。出す面 = (a) poll が `failed` を返したとき (b) action が `in_progress` を返したとき (c) `/app/upload` 再訪時の処理中カード(`page.tsx:53` `hasActiveProcessingUpload` の server-side guard が UploadForm を隠す面)— S-4 で 3 面とも同趣旨に揃える。
