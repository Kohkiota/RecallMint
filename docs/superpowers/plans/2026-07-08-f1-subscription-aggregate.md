...
- **挙動変更は Task 7(W)のみ**。G/R は挙動不変 — 既存 golden/snapshot + G1-G7 の**更新ゼロ**が客観証明。golden 赤 = 即停止(golden を直して通す行為は禁止)。
...
- review: 全 code task = canonical(SDD task-reviewer・read-only)。**risk task(Task 4a webhook 配線 / Task 4b action 配線 / Task 7 W)= + Codex review**(codex-review.sh・未解決 Crit/Imp 0・上限 3 周)。
...
- phase 完了 gate(Task 6 後 = R 完了 / Task 7 後 = W / Task 8 = 最終): full `pnpm test` + `pnpm build` 追加。
...
## Phase R(4 commits: R1(Task2)→ R2(Task3)→ R3a(Task4a・webhook)→ R3b(Task4b・action)+ lint 独立 commit(Task6)。R3 を webhook/action で分割 = 決済 action の bisect 粒度向上。番号: 4a/4b が R3 の 2 slot、以降 lint=Task6 / W=Task7 / 最終=Task8)
...
- **目的**: spec §3.2/3.3 の純粋 aggregate と意図別 repository を**新設 + unit test**(既存コード未配線 = additive only。配線は Task 4a/4b)。
...
### Task 4a: R3a — 射影 use-case 新設 + webhook 配線置換(`refactor(stripe): F1-R3a use-case + webhook 配線 [reviewed]`・**risk task**)

- **目的**: `lib/stripe/project-subscription.ts`(spec §3.4)を新設し、handle-stripe-event の全 write site を aggregate/repo 経由に置換(挙動不変・webhook 側 bisect 単位)。
- **内容**: projectStripeSubscription(db, key, sub) = derivePlanFromStripe → anomaly notify → projectStripeSnapshot → repo.saveProjection → RETURNING gate 付き Clerk sync。3 射影経路(checkout Step2 = clerkId / created・updated = stripeCustomerId)を単一化、0 行 match 分岐(checkout=sync skip / created=silent / updated=notifyOps)は **caller 側に残す**。`.deleted` reset → applyDeleted + repo.applyDeletedReset。evaluateReleaseGate の判定部を aggregate.evaluateRelease に置換し、副作用(参照事実 A の表)を use-case が現行順序で実行。releaseCompletedDowngrade は**現 file のまま呼ぶ**(参照事実 B)。**action 側は本 task で触らない**(eager projection は W)。
- **制約**: throw 伝播不変(webhook = route outer catch → 200)。A-4 分岐(row-match/clerkId 分離)・mismatch(書かず notifyOps)・`.deleted` の currentPeriodEnd 非更新を verbatim 維持。route.ts / actions.ts 不触。
- **完了条件(= 旧 Task 4 中間検証を commit gate に昇格)**: per-task gate + R 系 gate(**route.test 全 suite / release gate / A-4 / contract / integration 全 green・snapshot 更新ゼロ**)+ canonical + **Codex**(risk task)Crit0/Imp0。

### Task 4b: R3b — action 配線置換(`refactor(stripe): F1-R3b action 配線 [reviewed]`・**risk task**)

- **目的**: actions.ts の gating と予約 write site を aggregate/repo 経由に置換(挙動不変・W の下準備)。**action は use-case を使わない**(eager projection は W)。
- **内容**: changePlan block 判定(:109-116)→ aggregate.canChangePlan / downgrade 予約 set(:143-147)→ repo.saveReservation / cancelDowngrade clear(:201-205)→ repo.clearReservation。**A-3 try/catch + notifyOps + rethrow 構造は actions.ts 側に verbatim 維持**(repo 呼出を包む形へ)。action 全分岐の pin = actions.test 37 本(A-3 系 6 / block 3 条件 + DB-col-truth regression 対 / resolve error 系)+ G5。
- **制約**: throw 伝播不変(action = rethrow)。CHANGE_BLOCKED/NO_CHANGE/NO_SCHEDULE 文言・redirect 位置・idempotencyKey 生成を verbatim 維持。route.ts / handle-stripe-event.ts 不触。
- **完了条件**: per-task gate + R 系 gate(**actions.test 37+G5 / A-3 suite / contract / integration 全 green・snapshot 更新ゼロ**)+ canonical + **Codex**(risk task)Crit0/Imp0。

### Task 6: R4 — import 境界 lint(`chore(lint): F1 domain import 境界 [reviewed]`)
...
### Task 7: W — eager projection(spec §6)
