...
- (d) boundary null 時に th/td の DOM が S4 時点と一致(追加要素・class なし)を 1 case 固定。
- full-dir green + typecheck/lint exit0 + canonical/Codex Crit・Imp 0 → `[reviewed]`。
...
**制約**: Global。resize 中の offset 追従は CSS 変数経由で担保(memo 凍結 body に JS 再計算を入れない = Fix-3 T1 パターン延長・spec D-5)。z 階層: pinned th = `z-10`(thead 内)/ pinned td = `z-[1]` + 不透過 `bg-background`。hover 色 = `group-hover:bg-[color-mix(in_oklab,var(--muted)_50%,var(--background))]`(spec D-5 確定・token 名照合済)。separator 判定は `column.getIsLastColumn('left')`(可視 leaf 基準 = hidden boundary で最右**可視** pinned 列に付く)。spacer `<tr>` は変更しない(spec D-7)。virtualizer・collapse・scroll 保持のコードに触らない。
**完了条件**:
