...
- **react-hooks/refs off の判定結果(Task4・2026-07-07)**: **据え置き**(解消せず)。use-card-options.ts の `optionsRef.current = options` render-phase 書込は Sync-fix-1 由来。解消には ref パターンの構造的再設計が必要で、それは再レンダリング/タイミング挙動を変える = P3 挙動不変原則に反する。Task4 は pure carve のみで ref パターン非接触。理由 + 次工程(Sync-fix-1 または P4 で ref 再設計と併せて解消)を eslint.config.mjs override block コメントに記録済(OT 条件 4)。
...
- **Task5 縮小 fallback の判断結果(2026-07-07)**: **縮小採用**(full props 抽出でなく)。header 行(sort_key/title)が構造的に genuine diverge — inline-card-list = flex-wrap 単一行・無 label・+DeleteCardButton / side-peek = 縦 stack・`<p>` label・無 delete。full 抽出だと layout/label/showDelete の 3 条件分岐を新設する羽目になる(縮小 fallback が明示的に警告した状態)。ゆえに verbatim tail(CardTagsSection → 問題文 → 選択肢 → 解説 → メモ)のみ `card-editor-fields.tsx` へ抽出し Fragment 返却(親 space-y-* 不変)、意図的差分(header/DeleteCardButton/autoEditOnMount wiring/layout)は各 caller 残置。共通 component は条件分岐ゼロ(10 props は全て pure data・landmine #1 が subscription 禁止ゆえ props 不可避)。canonical/Codex とも Approved。
- **custom-filter-form:21 の確認結果(Task8・2026-07-07)**: **解消済**(allowlist 除外)。custom-filter-form の filter 型 import(TagFilterValue 等)は現 HEAD で `@/lib/cards/card-filter-predicates` = P1 の lib/cards 移動で cross-feature でなくなった。残る study→exams は CardTagAddPopover value import 1 件のみ(allowlist entry 1)。よって cross-feature allowlist は当初想定 5 件でなく **4 件**(study→exams ×1 / exams→tags ×2 / column-pinning 逆依存 ×1)。
