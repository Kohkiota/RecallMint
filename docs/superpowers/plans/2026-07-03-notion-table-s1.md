...
- label 定数 `ANSWER_STATE_LABELS` / `STREAK_OP_LABELS` は S1-2 で condition-bar.tsx へ移設 export(filter-bar は import に切替、editors も import。理由: バー(S1-2)が editors(S1-3)より先に要るため 1 箇所に置く)。
- testid: sort chip = `condition-chip-sort-<columnId>` / filter chip = `condition-chip-filter-<columnId>` / すべてクリア = button text「すべてクリア」。
- `COLUMN_DISPLAY_NAMES`(condition-bar 内 const): 条件を持ちうる 5 列のみ = question「問題文」/ lastCorrect「直近正誤」/ currentStreak「連続正解数」/ lastReview「最終回答日時」/ tags「タグ」。
...
**目的**: `ConditionBar` 新設。`deriveConditions` で sorting+columnFilters を chip 列挙(sort chip: 「並び替え: <列名> ↑/↓」・click で方向 flip・×= `setSorting` filter 除去 / filter chip: 値要約 label・×= `setFilterValue(undefined)`。編集オープンは S1-3)。「すべてクリア」= 両 state 空化。条件ゼロ → `null`(シュリンク)。hidden 列の条件も表示(列名は bar 内 `COLUMN_DISPLAY_NAMES` 純関数 map・`table.getColumn(id)` は visibility 非依存)。`filterBarWrapperRef` 内(既存固定バーと一時共存)に mount。
...
**完了条件**:
- condition-bar.test(新規): ① sort 2 件 + filter 1 件で chip 3 個・配列順 ② sort chip × → 当該のみ削除 ③ sort chip click → desc flip ④ filter chip × → `setFilterValue(undefined)` 経路で行復元 ⑤ すべてクリア → 全行復帰 + バー消滅 ⑥ 条件ゼロ時 render なし ⑦ 列 hidden(columnVisibility)でも当該 chip 表示。
...
**制約**: Global(特に predicate 不変・value 形不変)。editor の値構築ロジックは既存 filter-bar からの移設(挙動同一・aria-label 維持)であり新規判定を書かない。`FilterEditorContext`(categories/options)は `ExamCardTable` が既に持つ filter-bar 向け props をそのまま流す。固定バーはまだ削除しない。tags の `allAssignedOptionIds`/`onToggle` 配線は filter-bar の adapter 実装を editor へ移設。
**完了条件**:
- filter-editors.test(新規)= 既存 `exam-card-table-filter-bar.test.tsx`(3 describe)の **port**: 回答状態(header menu 経由で絞り込み・'all' で解除)/ 連続正解数(≤ 入力で絞り込み・空で解除)/ タグ(popover 選択で絞り込み・chip × で解除)— 絞り込み結果の期待値は旧 test と同値(non-vacuous)。+ chip click 編集で値変更が反映される 1 件。+ selectOnly で作成/編集導線が出ない 1 件。
- 既存 filter-bar.test はこの時点も green(共存)+ suite 全 green → review → `[reviewed]` commit。

### S1-4: 適用中インジケータ

**目的**: registry 登録列 header に filter dot(`column.getIsFiltered()` 読取のみ・aria-label「フィルタ適用中」)。**描画は th レベル**(既存矢印と同じ span 内)= trigger 種別(menu / tags 直 popover)に依存せず tags 列にも同経路で出す。sortable 未ソート時の `⇅` を menu affordance(chevron)に置換(ソート中 ▲/▼ は不変)。
...
- 既知リスクと引当:
  ...
  - 初期 `sorting=[]` の順序回帰 → データ pre-sort(:232)が担保、既存 exam-card-table.test で検出可(S1-1 完了条件)。
