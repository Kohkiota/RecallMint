...
### 3.5 sentinel 追跡台帳(§4 判定・**listing のみ / 手動削除は絶対にしない**)

前回 sentinel が手動回収されて実験が不成立になった経緯があるため、**回収せず期限まで放置**する。

| sentinel | PUT(lastModified) | 24h 到達 | **48h 境界(failure 判定線)** |
|---|---|---|---|
| A(8/8 分)`f4f91e6d…/5ff46746…` 269,867 B | `2026-08-08T11:44:57Z` | 2026-08-09T11:44Z | **2026-08-10T11:44Z** |
| B(8/9 分)`55b4316c…/96fe104d…` 344,798 B | `2026-08-09T01:09:31Z` | 2026-08-10T01:09Z | **2026-08-11T01:09Z** |

判定規則: 境界より前に消えれば **lifecycle 実削除の実測**(消失を確認した観測時刻と、直前に存在を
確認した時刻を併記して消失時刻の上下界を残す)/ 境界を越えて残っていれば **failure 判定**。

**観測ログ**

| 観測時刻(UTC) | A | B | 判定 |
|---|---|---|---|
| 2026-08-09T11:10:07〜11:26:01(2.5s 間隔・412 サンプル連続・欠落 0) | 在(age 23.7h) | 在(age 10.3h) | **判定保留**(両者とも境界前) |
| 2026-08-09(時刻未記録・**OT が Cloudflare dashboard で確認**) | **不在** | — | OT 報告 = 境界前に消失 |
| 2026-08-09T23:45:35Z(**CC が `listObjectsWithMetaBounded('src/')` で readback**) | **不在** | 在(age 22.60h・`lastModified=2026-08-09T01:09:31.220Z` = PUT 記録と完全一致) | **A = lifecycle 実削除の実測**(下記) |

## 判定(確定・2026-08-09)

**sentinel A = lifecycle は効いている(実削除の初実測)。**

- 消失時刻の上下界: **最後に存在を確認 = 2026-08-09T11:26:01Z(age 23.7h)** / **不在を確認 = 2026-08-09T23:45:35Z(age 36.0h 相当)**。よって消失は age **(23.7h, 36.0h]** の区間で起きた
- 24h 通過(2026-08-09T11:44Z)の**後**、48h 境界(2026-08-10T11:44Z)の**前**であり、判定規則どおり **lifecycle 実削除**。前回 sentinel(2026-08-07 分)が 24h 未満で消えて「rule の効果では説明できない」= 実験不成立だった(§3.4)のと違い、今回は 24h を跨いでいるため rule 由来として説明がつく
- 観測は 2 経路(OT の dashboard 目視 / CC の listing readback)。**厳密な消失時刻は取れていない**(polling を継続していないため)。得られたのは上下界のみで、「典型 24h 以内」という無保証仕様の範囲内であることの確認まで
- **B は保護解除**(OT 裁定 2026-08-09)。§3 sweeper の stg smoke で回収されてよい — 回収されればそれ自体が sweeper 実削除の実証になる。B は age 22.6h で 24h 通過が 2026-08-10T01:09Z ゆえ、smoke 前に lifecycle が先に消す可能性もある(その場合は 2 例目の lifecycle 実測として記録する)

**帰結**: spec `2026-08-09-ocr-2-4b-s3-src-sweeper-design.md` §11 の分岐は「**lifecycle は効いている**」で確定 — sweeper = 二次回収 + 期限超過検知 / lifecycle = backstop。`docs/architecture.md` / `docs/harness.md` も同日確定済。

> ⚠ **`CRON_SECRET` の順序則は解除済**(2026-08-09 判定完了)。判定前は「stg に設定するな」だったが、
> A の判定が終わり B は保護解除されたため、stg への設定は可。
