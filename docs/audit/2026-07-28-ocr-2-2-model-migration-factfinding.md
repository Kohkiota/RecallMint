...
## §6 OT 確定事項(2026-07-29)

- **論点 1**: thoughtsTokenCount 本体 fix を ②-2 に**含める**。ただし**モデル移行とは別 commit**(変更源が別・片方のみ revert しうる)。
- **論点 2 = (b)**: ②-2 は**観測のみ**。本文 markdown 画像混入の描画側 enforce は**別 sprint(②-3 系)へ持ち越し**。理由: ②-2 の目的は「モデル変更で壊れない」確認で、描画変更を混ぜると異常時の起因切り分け不能。②-3 と同じ描画側単一点ゆえ二度触らない。壊れ画像なし + 実ユーザー 0 で ②-2〜②-3 間の本文汚染は無害。
  - **②-3 持ち越し時の性質記録(重要)**: 「cosmetic な癖」ではなく「**target 単位契約に対する lite の逸脱を描画側で強制する**」と記す。前者だと見栄え調整、後者だと**契約として test で固定**する形になり設計が変わる。
- **論点 3**: ModelKind は最小温存(`'flash'` を主 OCR モデルのラベルとして残しコメント補足)。**実体のモデル ID を二重に書かない**(片方が腐る)。「実体は modelId() の定数を参照」の形。
- **プロンプトの「プレースホルダ埋め込みについて」行**: 将来削除で可(消極的打ち消しで lite に効いていない・実測根拠なし)。ただし ②-2 は prompt 凍結ゆえ**本 sprint では触らない** → ②-4 へ(下記)。

## §7 ②-4 持ち越し(prompt 画像関連の整理・②-2 では触らない)

②-4(図版切り出し)が prompt を触る際に**まとめて一度で**処理する(同一箇所ゆえ最安)。②-3 は描画側単一点で完結し prompt を触らない。prompt の画像関連整理は ②-4 に集約:

1. **`IMAGE_REFERENCE_RULES` 冒頭コメント**「画像本体の中身解釈はしない(AI は別冊画像を切り出せない)」(`ocr-extract.ts:108,115`): 別冊(手元に無い別冊子)は今も正しいが、**同一ページ内の図については誤り**(②-0 で box_2d 座標取得を実証)。②-4 でこの前提記述ごと書き換え。
2. **`COMMON_EXTRACTION_RULES` の「画像は抽出しない」**(`:233`): ②-4 はこの方針変更そのもの。書き換え漏れると「図を検出し座標を返せ」と「画像は抽出しない」が prompt 内で矛盾同居。
3. **「プレースホルダ埋め込みについて」行**(`:152-155`)の削除: 上記 1・2 と同一箇所ゆえ ②-4 で一括処理。
