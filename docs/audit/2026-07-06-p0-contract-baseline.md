...
| P2 | **演習回答 flush → 集計**(SessionRunner 完走 → review-events/bulk 経路 → dashboard の streak/today count/正答率が反映)/ **upload OCR → preview**(upload → guard tx〈advisory lock・quota・in-flight〉→ OCR → cards+tags 保存 → result preview 表示)/ **決済 webhook 経路**(stripe subscription 状態遷移が users 行に反映・upgrade/downgrade。stg は Stripe CLI 転送のみ)/ **アカウント削除経路**(Clerk user.deleted → Stripe cancel + 10 テーブル DELETE + soft delete。実削除は OT 実機領域) | 各 surface が P2 前と同一挙動か: 演習 flush 後の dashboard 数値・正答率一致(FSRS ingest が lib/reviews へ移動しても replay 結果不変)/ upload の guard 分岐(処理中・quota 超過・上限)と成功時 preview 一致 / webhook 反映後の plan/status 表示一致(**実課金は OT**)/ アカウント削除の子データ全消し(**破壊的=OT 実機**)。契約面(payload・error code・文言・revalidatePath 等)は P0 golden が既に担保 — smoke は「移設後も実挙動が同一か」の確認が主 |

| P3 | **タグ CRUD 実操作**(exams カード詳細 / 一覧タグセル / タグ popover / side peek でのカテゴリ・option の rename/color/create/delete/impact 表示・楽観反映 + rollback)/ **card inline 編集**(問題文/選択肢/解説/メモの inline 編集 → 空文字→null 正規化 → mirror + outbox → flush・commit-on-unmount・side peek と一覧の両レイアウト)/ **side peek 開閉 commit**(option cell 編集中の Esc/× close で blur→commit が走る=V6)/ **tags manager**(/app/tags のカテゴリ・option の cascade delete が exams と単一 use-case 経由・削除確認 dialog・active 解除)/ **selection prune**(一覧の複数選択が filter 変更・行削除で可視集合に追従) | 各 surface が P3 前と同一挙動か: タグ操作の楽観反映と失敗時 rollback(Dexie auto-rollback)/ inline 編集の正規化結果・commit タイミング・side peek ⇄ 一覧の描画一致 / V6 の close 時 commit(編集値が保存される)/ manager 削除の cascade と exams 側の一致 / selection の prune 挙動。契約面(patch shape・enqueue 順序・op 名・error code・文言)は P0 golden + contract 77 snapshot 更新ゼロで担保済 — smoke は「client interaction の実挙動が同一か」の確認が主。**破壊的削除(カテゴリ cascade)は OT 実機領域**。<br>**E2E 残余(重要・E2E 不採用の意図的 stg 送り)**: jsdom + fake-indexeddb + RTL の統合 test では担保**不能**な 3 点を merge 後 stg smoke(DevTools MCP 実 browser)で撃つ — ① **実 focus/blur 挙動**(jsdom は activeElement/focus dispatch を実装せず・V6 の blur→commit の実フレーム差): 操作 = side peek で option cell を編集中に Esc/× で閉じる / 期待 = 編集値が commit されて保存される。② **virtualizer 実 scroll**(jsdom layout=0 で getBoundingClientRect=0・実 window 追従不能): 操作 = 300+ カードの一覧を scroll / 期待 = memo 凍結が効き cell 再描画されず列幅・pinned 列が実時間追従。③ **scroll 起因の実 unmount commit**(実 scroll tear-down は RTL unmount と異なる): 操作 = inline 編集中に該当 cell が virtualizer window 外へ scroll / 期待 = commit-on-unmount で編集値が保存される。※ この 3 点は P3 で E2E を新規導入せず(新規依存・費用対効果・spec §3.2)stg smoke へ送った意図的判断であり、「P3 で実 focus/blur を test してない」は漏れでなく設計。 |

| P4 | **インフラ DRY(挙動不変・UI surface は原則なし)** — server/lib の同型コピペ集約 + lib 再編。触った実挙動経路: **統合 /api/pull**(Task2 pull-delta factory 化・6 stream delta 取得)/ **read-only 4 route**(Task3 wrapper: /api/pull・/api/dashboard/stats・/api/study-days/pull・/api/exams/status の認証・空応答・no-store)/ **outbox flush**(Task1 per-table helper: entity-mutations ⇄ review-events の sync/attempted/dropStale・bulk POST)/ **決済・認証 fail-fast**(Task4 clerk/stripe env-check 移設・module-load throw)/ **演習 flush 応答**(Task8 measure 撤去後の review-events/bulk)/ **contact form 送信**(Task7 server action lib 移設) | 各 surface が P4 前と同一挙動か: **pull 同期**(初回 full pull + 増分 cursor pull で cards/exams/tags/card_tags/tombstones が同一件数・同一 cursor 前進・card_tags 例外〈カード更新起点の旧行削除+再 upsert〉が効く)/ **read-only route 応答**(認証時の正常 body・未 sync 時の空 200・401・no-store header が 4 route で P4 前と同一)/ **outbox flush**(演習/編集の楽観書込 → flush で synced 遷移・失敗時 backoff〈非対称の retry controller は不変〉)/ **決済・認証起動**(env 不足時の fail-fast throw が deploy 時に現行と同一入口で発火 — 実 key での起動確認)/ **演習 flush**(timing log 撤去後も回答集計・dashboard 反映が不変)/ **contact 送信**(フォーム送信 → contact_messages 保存・rate limit・honeypot)。全て契約面は P0 golden + contract 77 snapshot 更新ゼロで担保済 — smoke は「移設・DRY 後も実挙動が同一か」の回帰確認が主。**実課金・破壊的操作・実 key fail-fast は OT 実機領域**。P4 は UI 変更を伴わないため P1〜P3 のような新規 UI surface はなく、既存経路が壊れていないことの確認に尽きる。 |

**注記**: P1〜P4 は wire/契約(payload・error code・文言・revalidatePath・op 名・既存 log 名等)を一切変えていない(D-2 凍結契約不変・各 phase の whole-branch review + contract 77 snapshot 更新ゼロで確認済。P2 の measure TEMP marker〈P4 Task8 で撤去済・timing/logger 非凍結〉・P3 の Option B error-path warn〈sibling *.tx_failed 命名内〉は sanctioned な例外)。ゆえに上記 smoke は「表示・並び・数値・client interaction の同一性」確認が主で、契約面は P0 golden が既に担保している。P3 は client interaction が P0 golden の cover 外のため、Task0 characterization(selection prune / TagCell override / delete reject / cross-component 統合 flow)+ 既存 co-located suite が回帰網の正 — stg smoke はそこも cover しない実 browser 依存 3 点(上記 E2E 残余)に絞る。
