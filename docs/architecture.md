...
- **共有ブラウザの残骸(at-rest)**: 表示漏れは **correctness sprint(2026-08-16)で解消**(読みの owner スコープ化 + owner namespace — §1 の owner 分離不変条件の行)。**残るのは IDB / Cache API に前 user の行が残ること自体**で、除去は hygiene sprint(sign-out purge / 異 owner sweep / Cache cleanup / 旧 key 物理削除)に送った。**「表示されない」は構造保証だが「消えている」は保証しない**(spec §7 の三層)。共有端末のローカル利用者が DevTools で IDB を直接見れば前 user のデータは読める — これは UI 経路の tenant isolation とは別の残余リスク。
