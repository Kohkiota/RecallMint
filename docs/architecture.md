...
| **`answer_events.applied = true` ⟺ `review_logs` に対応行が存在する(同一 tx の原子性で成立)** | ts-fsrs の ReviewLog を event 単位で永続化し、将来の FSRS 再計算・監査に使える形で保持する(R0「ReviewLog 持続化」) | 証明: `tests/integration/pg/review-logs.test.ts` の ①(1:1)③(applied=false → 0 行)⑥(失敗注入で全 rollback)。決定 2026-08-16 | `lib/reviews/ingest-review-events.ts`(手順 7.5)/ `docs/superpowers/specs/2026-08-16-r0-review-log-persistence-design.md` |
...
| **client 持続状態の owner 分離は「空間的」に行う(時間的排他=lock で解かない)。① owner 列を持つ store は行の `user_id` で分離 ② 持たない `sync_meta` は key を `${base}:${userId}` に名前空間化し、構成は必ず `scopedSyncMetaKey` を経由 ③ pull は開始時に capture した userId を cursor の read/write 両方に使う(mutable な「現在の user」を完了時に参照する実装は禁止)④ pull 応答は `owner_user_id` echo + owner 列を持つ 5 stream の全行 owner を **tx を開く前に**検証し、違反は全体 reject** | **時間的排他は「race 参加者の完全列挙」という完全性主張を要求し、反例駆動で 3 周崩れた**(spec r1〜r3)。空間的分離は遅着 writer が何本居ても成立する。書込点の全数棚卸しで要保護対象が `sync_meta` と `study_days` 全置換の 2 点に閉じることを確認した上での転換 | 証明: `lib/sync/pull.test.ts`(cursor namespace 6 stream table-driven / capture 原則違反の実装形で red になる pin / owner echo 4 形 / validate-before-tx の順序 pin)/ `lib/sync/study-days.test.ts`(owner 限定置換・mixed reject)/ `lib/sync/sync-meta-access-audit.test.ts`(許可 file 限定 audit)。決定 2026-08-16 | `docs/superpowers/specs/2026-08-16-tag-mirror-owner-scope-and-signout-purge-design.md`(r5)/ `docs/superpowers/sessions/2026-08-16-tag-mirror-writer-inventory-factfinding.md` |
| **上記の機械強制は部分的**: `scopedSyncMetaKey` 経由の規約は「production の `.sync_meta` 直接 access を許可 file(`lib/sync/sync-meta.ts` / `lib/sync/pull.ts`)に限定する」audit test で**どの file が触るか**までしか検出しない — 許可 file 内での key 構成の正しさは見ない。また audit は `git ls-files` ベースゆえ **untracked な新規 file は `git add` まで検出できない**。残りは規約 + review 観点 | 完全な lint 化は過剰(YAGNI)。限界を隠さず記録する方が、後から「機械強制がある」と誤読されるより安全(`docs/architecture.md:106` の server 行ロック順規約と同型)| 決定 2026-08-16 | `lib/sync/sync-meta-access-audit.test.ts` |
| **本 owner 分離保証の開始点は「correctness sprint 以降の bundle を実行している tab」に限る**。deploy 前から開いたままの旧 bundle tab は unscoped cursor 書込 / `study_days.clear()` / owner 無スコープ表示を続けるため保証外(旧 tab の一時的な study_days 消失は新 tab の次 pull で自然回復)| 全 tab の即時 reload を強制する機構(version 検出 + 強制 reload)は編集中データや pending outbox を失うリスクがあり、得られる保証に見合わない | 決定 2026-08-16(受容 + 記録)| 同 spec §7 |
| **roll-forward が load-bearing**: 本改修の rollback は「旧版が別 user 由来でありうる legacy unscoped cursor を読み直す」= 解消した cursor 汚染と under-fetch を再開させる。「over-fetch 方向で冪等だから安全」ではない。server のみ戻すと新 client が `owner_user_id` 欠落で**全 pull を silent reject** する | rollback 時の互換 patch(旧版が読む前に legacy unscoped cursor 6 本を削除して full pull を強制)が必須。patch は事前に作らない(必要時) | 決定 2026-08-16 | 同 spec §7a |
...
| **hygiene(sign-out purge / sign-in 異 owner sweep)の Dexie 削除は、触る全 store(mirror 6 + sync_meta + outbox 2 + media 2 = 11 store)を跨ぐ単一 rw tx で行い、owner を問わず触らない不可侵集合を持つ**: ① pending / syncing / failed の outbox 全行 ② 非 `'ready'` の `media_assets` 行 + 対応 Cache blob ③ `'downloading'` の `media_download_jobs` 行 + `added_asset_ids` の Cache blob。**削除条件は陽形のみ**(`media_assets`=`'ready'` / `media_download_jobs`=`'done'` / outbox=`'synced'`)— 否定形(`!== 'downloading'` 等)は将来追加された status を silent に削除対象へ入れるため使わない。**purge / sweep 自体は best-effort**(発火・順序・完了待ち保証なし)で、eventual は「次回実行機会があれば収束する」の条件付き保証(時間上限のある無条件回収保証ではない) | 単一 tx は「mirror だけ消えて cursor が残る」部分実行状態(同一 owner の再 sign-in で delta pull が消えた行を取り直さず silent 表示欠落を起こす)を構造的に禁じ、次行の pull cursor CAS が意味を持つ前提でもある。不可侵集合は棚卸し doc に無かった新規 hazard から導出: `entity-mutations.ts` の flush gate(`collectBlockedImageMutationIds`)は `media_assets` の `'uploading'` 行の存在を根拠に pending images mutation を保留しており、行だけ消すと gate が開いて実体の無い image key を server に確定させる(同原則を DL レーンの `'downloading'` job にも適用)| 証明: `lib/sync/local-hygiene.test.ts`(分類 table-driven pin + fake-indexeddb 実走 pin + tx 原子性 pin(spy throw で全 store 変更前のまま)+ 網羅 pin)。決定 2026-08-17 | `lib/sync/local-hygiene.ts` / `docs/superpowers/specs/2026-08-17-tag-mirror-hygiene-design.md` §0・§4 |
| **pull の apply tx は先頭で cursor CAS(compare-and-swap)を行う**: §1 で cursor 6 本を読んだ時点の snapshot(`undefined`=不在も値として含む)と、apply tx 内で同 6 key を再読した値を厳密比較し、1 本でも不一致(値変化・消失・出現)なら tx を書かずに abort する。abort は sentinel error で表現し、pull の外へは漏らさず既存の silent FAIL 契約(`{ok:false}`)へ正規化する。**CAS は owner 検証ではなく「読取から tx までに store が動いたか」を見る並行性検証**で tx 内でしか意味を持たない(owner 検証は validate-before-tx の凍結規律どおり tx を開く前に完結し、両者は役割が異なるため矛盾しない) | purge / sweep が cursor 読取〜apply tx の network 窓に挟まると、遅着した「旧 cursor 由来の delta」が空になった mirror へ apply され新 cursor が書かれ、purge で消えた行が永続的に silent 欠落する(correctness sprint が解消した under-fetch の再発)。**CAS は「窓中の変更検知」であり、pull 開始時点で既に mirror/cursor が不整合な場合(旧 bundle 由来等)を修復する機構ではない**(mirror reconcile は非スコープ)| 証明: `lib/sync/pull.test.ts`(値変化・消失・出現の 3 態で abort・mirror/cursor 不変・`{ok:false}` / abort が例外として漏れず FAIL 正規化される pin)。決定 2026-08-17 | `lib/sync/pull.ts`(`CursorCasMismatchError`)/ 同 spec §3 |
| **sign-in sweep における `sync_meta` の削除判定は明示リテラル allowlist**(`SWEEP_SYNC_META_BASES` = cursor 6 本 + 旧 `exam_view_prefs` の 7 本)で行い、`SYNC_META_KEYS` から自動導出しない。各 key は base の bare 一致→delete / `base:<userId>` で self 一致→keep・それ以外(空 suffix・複数 colon 等の malformed 含む)→delete / どの base にも非該当(prefix 類似・未知 key)→keep。**分類強制 pin**が `SYNC_META_KEYS` の全値が allowlist ∪ 明示除外 list に現れることを assert し、新規 key 追加時に「sweep 対象か否か」の判断を明示的に踏ませる。**sign-out purge は同 store を未知 key 含め全消し**で、sweep との非対称は意図的(去る側に残骸を持つ権利はない/sign-in 側は将来 key を silent に誤削除しない) | 「自分の key 以外を全部消す」形にすると将来 key が追加されるたびに sweep が黙って消し続ける regression を作るため、allowlist を明示リテラルに固定し追加時の判断を強制する(Codex 事前指摘)| 証明: `lib/sync/local-hygiene.test.ts`(allowlist pin + parser 境界 pin + 分類強制 pin)。決定 2026-08-17 | `lib/sync/local-hygiene.ts`(`classifySyncMetaKeyForSweep` / `SWEEP_SYNC_META_BASES`)/ 同 spec §5.1 |
| **deck DL(`downloadDeckImages`)は両方の成功出口を、deck 全 key の blob 現存検証(success gate)で支配する**: ① 全件 preflight hit の早期 `ok:true` ② DL 後の `'done'` 確定を経た `ok:true`。**検証対象は preflight hit 分を含む deck 全 key**(`verifyDeckBlobs`)— hygiene cleanup と進行中 DL の交差で「`ok:true` + blob 欠け」を成立させない(受容ではなく構造的に閉じる)。早期出口は job row / added blob とも未生成のため rollback 対象がなく結果を返すのみ、DL 本体経路は既存 rollback(added blob + job row 削除)に合流する | hygiene の保護 blob 集合は「Dexie tx 後に観測された行」から算出するため、算出後に開始した進行中 DL の blob は cleanup に巻き込まれうる(TOCTOU)。upload レーンは R2 PUT の source が in-memory blob で実害が表示 cache miss(再 fetch で回復)に bound され受容できるが、deck DL の all-or-nothing 契約は blob 集合の完全性を含むため受容できず、DL 側の gate で閉じる | 証明: `lib/media/deck-download.test.ts`(出口別・個別変異(全件 hit / hit 分削除 / added 分削除)の gate pin)。決定 2026-08-17 | `lib/media/deck-download.ts`(`verifyDeckBlobs`)/ 同 spec §4.2 |
| **上記 hygiene 層の機械強制は部分的**: `local-hygiene.test.ts` の網羅 pin は「`ClientDb` の全 store が `HYGIENE_STORE_RULES` に purge/sweep 両規則付きで宣言されていること」までしか検出せず、宣言した規則の値の妥当性は store ごとの個別 `toEqual` pin(手動追加)に依存する。sign-out purge / sweep の mount 起動点(`app/layout.tsx` / `app/(app)/app/layout.tsx`)は `sign-out-purge.test.ts` / `hygiene-sweep-trigger.test.tsx` の **source-text マッチ**(`layoutSource.toContain('<SignOutPurge />')` 等)で pin しており実 render は行わない — **JSX の表記(self-closing / 属性追加 / import 経路の変更)を変えると偽陰性になりうる**(`sync-meta-access-audit.test.ts` と同型の限界) | 完全な lint 化は過剰(YAGNI)。限界を隠さず記録する方が、後から「機械強制がある」と誤読されるより安全(本表 sync_meta access audit 行 / §8 の server 行ロック順規約の限界記述と同型)| 決定 2026-08-17 | `lib/sync/local-hygiene.test.ts` / `app/_components/sign-out-purge.test.tsx` / `app/(app)/app/_components/hygiene-sweep-trigger.test.tsx` |
...
| **`review_logs` の不変条件に対する 3 つの証明の空白**(§1) | 中(R0・新規 2026-08-16)。① **deploy の新旧混在窓と code rollback 中は成立しない**(旧 code は log を書かない。coverage 契約 = 欠損期間を記録し L4 消費時に除外/注記)② **`review_logs.user_id` と参照先 `answer_events.user_id` の一致は DB 保証がない**(複合 FK は `answer_events` への UNIQUE 追加を要し凍結契約に抵触するため不採用。代替保証 = 同 iso ⑦(a)(b))③ **anti-join 監査(applied=true AND NOT EXISTS log)は未実装**(follow-up)。正本 = `docs/superpowers/specs/2026-08-16-r0-review-log-persistence-design.md` |
...
- **共有ブラウザの残骸(at-rest)**: 表示漏れは **correctness sprint(2026-08-16)で解消**(読みの owner スコープ化 + owner namespace — §1 の owner 分離不変条件の行)。**除去は hygiene sprint(2026-08-17)で sign-out purge / sign-in 異 owner sweep / Cache cleanup / 旧 key 物理削除として実装した**が、保証水準は **eventual hygiene**(best-effort・発火/順序/完了待ち保証なし)であり「完全消去」は主張しない。**残置集合は §1 の不可侵集合どおり**(過小記載しない): ① pending / syncing / failed の outbox 全行(owner 不問)② 非 `'ready'` の `media_assets` 行 + 対応 Cache blob ③ `'downloading'` の `media_download_jobs` 行 + `added_asset_ids` の Cache blob。**うち異 owner の `'downloading'` job + added blob は既存 sweeper(`sweepStaleMedia` は自 owner scope — `lib/media/sweep.ts:119`)の対象外で、当該 owner が再 sign-in しない限り恒久的に残る**(一時的な残置ではない)。**うち pending / failed の at-rest 残置解消は公開前トラック**(flush-before-signout と併せ再裁定)。**「表示されない」は構造保証だが「(不可侵集合が)消えている」は保証しない**(correctness spec r5 §7 の三層 / hygiene spec §0)。共有端末のローカル利用者が DevTools で IDB を直接見れば、不可侵集合に該当する前 user のデータや purge/sweep 未発火時の残骸は読める — これは UI 経路の tenant isolation とは別の残余リスク。
