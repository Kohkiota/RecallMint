...
- **claim 応答喪失時の 15 分 in_progress**(②-4a-cutover 案 D・2026-08-02・OT 明示受容 = bounded residual)。**発動条件 = 公開前、または retry worker を実装する時**。claim が server commit した直後に通信断 / parse throw が起きた場合のみ、claimed op が valid lease のまま最大 15 分残る(実行中でないのに「実行中」表示)。self-heal(15 分後の supersede が掃除)。完全解消には operation 状態の再取得往復が必要。cutover 前は無期限 block だったため、現状は厳密に改善している。**受容理由**: fencing が正しく機能した帰結(valid-lease op を token 無しで外部から terminal 化できるなら実行中 worker を踏み潰せる = fencing の穴)。往復追加は実ユーザー 0 で YAGNI・fencing 弱化は安全性劣化ゆえ論外。§6.5 と同型。詳細 = spec §3.1 / `docs/superpowers/sessions/2026-08-02-ocr-2-4a-cutover-review.md` §7 / `upload-form.tsx` claim 段 comment。
