...
| `src/` age-based sweeper(日次 Vercel Cron `0 18 * * *` = 03:00 JST・**発火は production のみ**・**repo 初の cron**)| `src/` に残った期限超過 PDF(§1 staging DELETE の取り漏らし / 所有権喪失 skip / §2 退会 purge の打ち切り残 / finalize reject の削除失敗)。cutoff 6h・age で判定し、`src/{uuid}/{uuid}/{uuid}.pdf`(case-insensitive)一致かつ live-op を持たない user の object だけを消す。**`CRON_SECRET` 未設定 = 何も掃かれない**(fail-closed。secret 不在が「壊れる」でなく「守る」側に倒れる反転は非自明ゆえ明記)。**応答は tier で違う** — preview / local は空 secret gate が弾いて 401、production は `requireWebhookSecret` 自身が throw して 500(外周 catch)。「掃かれない」は両 tier で真だが切り分け時に 401 だけを期待しない。`?cutoffMinutes=` の override は production では 400。**HEAD も Next 16 の auto-implement で同一 handler = valid secret 付き HEAD は実削除を走らせる**(冪等だが health-check に使わない)| 構成(cron・`vercel.json` crons)+ 機械(route の auth / 台帳)| `docs/superpowers/specs/2026-08-09-ocr-2-4b-s3-src-sweeper-design.md` / `app/api/cron/sweep/route.ts` / レーン間の契約 = `docs/architecture.md` §11 |
| `/api/cron/sweep` runner の per-lane 固定絶対 deadline(**同一 cron が `src_sweep` / `asset_gc` / `asset_orphan_scan` の 3 lane に拡張**・各 lane の deadline = run 開始時刻を原点とする固定オフセット `src_sweep` 90s / `asset_gc` 210s / `asset_orphan_scan` 260s・`vercel.json` の path/schedule は不変)| 先行 lane が共有予算を使い切り後続 lane が毎日 0 実行になる silent starvation(先行側の incomplete しか鳴らず、後続が止まっている事実自体は観測に出ない)。開始時点で残 slice が floor 未満の lane は `notStarted` を立てて起動しない(「実行されず」と「実行して 0 件」を区別)| 機械(runner)+ 構成(cron)| `app/api/cron/sweep/run-lanes.ts` / spec `docs/superpowers/specs/2026-08-10-asset-lane-gc-design.md` §2.1 |
| `asset_gc` lane(日次 cron・上記 runner の 2 lane 目・per-user 実行)| refs ゼロ化した画像 asset の R2 実体・行の回収漏れ(reconciler が手動実行のままだった穴・旧 §11 未解決事項①)。mark/promote/collect を per-user に実行し、collect は `COLLECT_LIMIT_PER_USER=20` で user あたり 20 object/run に bound(**soft** — 退会 asset の実削除は ⌈N/20⌉ 日)| 構成(cron)+ 機械(route の auth / 台帳)| `lib/storage/asset-gc-lane.ts` / spec `docs/superpowers/specs/2026-08-10-asset-lane-gc-design.md` §3 / レーン間の契約 = `docs/architecture.md` §11 |
| `asset_orphan_scan` lane(日次 cron・上記 runner の 3 lane 目)| `assets` 行の無い R2 object(row-less orphan)の恒久残留(旧 §11「未整備」)。key 規約一致 + age(cutoff 7 日)+ live-op 無しの三重条件 + DELETE 直前の行不在確認で回収。観測は listing 上限 10 page 内の partial observation(上限到達は `list_truncated` で毎日鳴る)| 構成(cron)+ 機械(route の auth / 台帳)| `lib/storage/orphan-scan.ts` / spec `docs/superpowers/specs/2026-08-10-asset-lane-gc-design.md` §4 |
