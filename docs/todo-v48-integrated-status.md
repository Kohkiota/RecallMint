# RecallMint 統合ステータス todo v48(②-4a cutover〜smoke 統合)

- 版: **v48**(v47 = 2026-07-18〜21 RLS セッション統合版を本体とし、本 doc は ②-4a-cutover〜6回目 smoke sprint の成果を差分統合した完全版)
- 更新: 2026-08-02 / 種別: 全 sprint 横断ステータス + roadmap
- 位置づけ: claude.ai の view。**最終優先順位は OT 決定が優先**。各ステータスは実コード verify / closure doc / stg 実証に基づく。
- **採番の注記**: 本 repo の現行版は v47(OT kickoff が呼称した「v49」は OT 私的採番で repo より ~2 版先行)。本新版は repo lineage を継いで **v48**。OT 側採番に揃えるなら rename 指示を(判断必要)。
- **v46 突合の注記**: 「todo v46」は OT 管理で本 repo に無い。参照正本: 各 closure doc / spec / plan(下記 path)。**v46 固有の OT 項目(未 repo 化の backlog / 優先順位)は OT が反映**する前提。
- **本版の差分**: 下記 **§0.5 = ②-4a-cutover〜smoke sprint** を新設(完了消し込み + 順序付き残タスク + 起票済み cross-ref)。§1-6 は v47 の RLS/deps/DDD/画像GC 履歴を carry-forward(cutover backlog line は §0.5 へ集約)。
...
## 0.5 ②-4a 画像図版切り出し(cutover〜6回目 smoke)= 直近セッション成果 ★本版の主差分

**位置づけ**: ②-4a server-side 実装(T1-T14a)は前セッションで [reviewed]/tagless 完了済。本 sprint は **UI 配線(cutover)+ 実環境 e2e smoke(実測の土台)** を入れた。正本 ledger = `.superpowers/sdd/2026-07-30-ocr-2-4a-image-figure-crop/progress.md` / handoff = `docs/superpowers/sessions/2026-08-02-ocr-2-4a-cutover-handoff.md`。

### ✅ 完了(消し込み)
- **②-4a-cutover(案 D = abandon + supersede・client resume なし)実装** = `19f34cb`(TAGLESS)。upload-form を新 flow へ配線(圧縮→prepareUpload→per-source{reserveSource→client PUT→finalizeSource}→claimOperation→stagePrepared→publishPreparedUpload→既存 result page)。案 D の詳細裁定 = review doc §7-8 / spec §3.1。
- **6 回目 smoke が実環境完走**: 5 枚 → 11 cards + 図版 attach(happy path e2e が実 Vercel 環境で成立)。所要 ~1-2 分/5 枚(auto-nav follow-up 起点)。
- **T10 / T12a / T12b / T14a の stg-smoke gate 達成** → **[reviewed] は session doc を正記録**(全 commit push 済=commit message tag は追わない・CLAUDE.md stg-smoke discipline)。本セッション Task 1 で正記録する(下記 残タスク #1)。
- **実環境限定 500 の 3 種を全 close**(いずれも local build/test/lint で原理的に検出不能):
  1. sharp libvips `.so` の NFT トレース漏れ → `fb65412`(next.config `outputFileTracingIncludes`)。診断=nft.json の `.so` 有無。
  2. `'use server'` file の型 named re-export を Turbopack が server reference 登録→裸参照→500 → `71c2e05`(型 re-export 削除 + **eslint Block E2-useserver-typeexport で機械強制**)。
  3. R2 server PUT の Content-Length 欠落→411 Length Required → `0171a6c`(headers に Content-Length 明示・RED→GREEN)。診断=Vercel Function trace の「External APIs」表示(smoke 手順書 §4)。
- **maxDuration 300s 消滅事象(2 回目 smoke)→ 6 回目で非再発ゆえ observe-close**。build は `/app/upload` に maxDuration=800 emit 済(functions-config-manifest.json)。根因(Vercel Fluid/Default 設定 疑い)は未解明だが非再発ゆえ深掘りせず close(再発時=Vercel Functions タブの実 Max Duration 確認 → 設定へ)。
- **stg DB 反映**: migration **0026-0030**(source_assets / upload_operations / asset_derivations / nullable 化 / expected_source_count)+ grants(base→phase3 順)+ RLS policy(`ocr-2-4a-enable.sql`)適用・runbook §2 検証 SQL 合格(RLS 3 表 on / policy 3 本 / GRANT 4 コマンド / expected_source_count 列)。**検証 SQL は schema 固定版(`docs/ops/ocr-2-4a-stg-migration-runbook.md`)を正とする**。

### 🔜 残タスク(順序つき・本 sprint 継続)
1. **②-4a-cutover close 処理**([reviewed] 正記録)= cutover/T10/T12a/T12b/T14a の smoke gate 達成を session doc に確定。commit tag は push 済ゆえ追わない。
2. **T14b: source_assets GC lane**(**破壊・stg-smoke gate**)。`gc-image-assets.ts` 方式で共通化 + stale temp `src/tmp/` delete。**完了条件に GC 整合 smoke を含む**: ① card A/B が同一 asset 参照 → A 削除 → asset は GC 非対象(refs 1 本残存)② A/B 両削除 → refs 0 → grace 経過後 GC。残骸源(PUT 成功後 finalize 前 temp / finalize 失敗 reserved / terminal op の ready source)を閉じる=**一般公開前の必須 gate**(ledger 記録)。
3. **T15: GDPR Group I 統合**(**破壊**)= 退会削除カスケードに `upload_operations` / `source_assets` を追加。
4. **T16**: 除外理由別件数の提示 + 回転除外(EXIF≠1)。
5. **follow-up 束**(smoke 後・T14b と並列可):
   - **表示 fix 案 Y**(claim-lost 非-live 化): `isLiveUploadOperationCondition` の `claimed` branch だけ tighten(claim-lost = claimed + lease 切れ + `next_retry_at IS NULL` + `last_error_code IS NULL` + `prepared_payload IS NULL` を非-live)/ `prepared`+payload の 7 日 live は維持 / reconciler が operation も terminal_failed 化(write 順序: op 先→DB `now()` 再検証→terminal+clear+`last_error_code='claim_lost'`→UPDATE 成功 op の doc だけ failed)/ **3 site(display/reconciler/sweep)real-PG RED→GREEN**。prepareUpload は変更不要(supersede が lease 切れ claimed を終端化)。詳細=review doc §8。
...
- **②-4b(PDF: rasterizer 検証 / Files API / exp7 回転 probe)/ ②-4c(DL・GC 統合)** = §5。cutover は ②-4b より前に必須ゆえ本 sprint で先行。

### ⏱ 時間予算(実測後確定)の現状
6 回目 smoke(5 枚 ~1-2 分)は全暫定値(`CROP_PHASE_BUDGET` 600s / `CROP_MIN_REMAINING_MS` 5s / sharp `.timeout()` 30s / OCR 720s)を大きく下回り deadline_excluded / timeout 未発火 → **暫定値は当該スケールで妥当性検証済**。精密な per-stage 確定(Gemini 応答 / crop 1 枚)は discrete 計測未取得 → **要 OT 判断**: 今確定するか実運用まで defer するか(deadline = per-invocation で確定済・operation 全体上限 = 7 日 retention cap)。
...
- **claim 応答喪失の 2 residual**(②-4a-cutover 案 D・2026-08-02・現物確認で訂正確定)。前提訂正: 「lease 切れ後も無期限 block」「status endpoint で解決」は**いずれも誤り**(外部レビュー由来・現物と食い違い)。現行 supersede が lease 切れ claimed を終端化するため、再アップロードは lease 切れ後に通る(`prepare-upload.test.ts` の expired-lease supersede test で green 実証)。
  1. **valid lease 中の block(最大 15 分)= 受容**。claim commit 直後に応答喪失時、lease が生きている間は claim-lost と stage 実行中が DB 上で完全に同一のため区別できない。heartbeat 無しでは原理的に解決不能。fencing の代償として受容。lease 切れ後は supersede が終端化するため再アップロードで続行できる。**発動条件(再評価)= heartbeat/retry worker を作る時**。
  2. **放置された claim-lost の「処理中」表示(最長 7 日)= 表示 fix で対処予定(下記・案 Y=smoke 後)**。ユーザーが再アップロードしない場合のみ発生する cosmetic。再アップロードすれば supersede が doc を failed 化して解消する。
- **[表示 fix — smoke 後・T14b と並列の独立タスク・案 Y]** claim-lost を素直に failed 化(「何も動いていないのに処理中」を放置ユーザーにも解消させる)。**発動条件 = smoke 後(実測結果を見て T14b との順序決定)**。内容: (a)`isLiveUploadOperationCondition`(`source-doc-status.ts:83-90`)の **`claimed` branch だけ tighten** = claim-lost(claimed + lease 切れ + `next_retry_at IS NULL` + `last_error_code IS NULL` + `prepared_payload IS NULL`)を非-live に。`awaiting_sources` の既存 window と `prepared`+payload の 7 日 live は維持。(b)`prepared` は締めない(payload あり=将来 retry worker で publish 再開可)。(c)reconciler は **operation も terminal_failed 化**・write 順序固定: operation を条件付き UPDATE(DB `now()` で再検証)→ terminal_failed + payload/lease/backoff clear + `last_error_code='claim_lost'` → **UPDATE 成功した operation の source_document だけ** failed 化(先に doc を failed にすると takeover された op と表示が食い違う)。(d)**3 site(display/reconciler/sweep)の real-PG test を RED→GREEN**(T14a null-lease 教訓と同型)。**prepareUpload は変更不要**(supersede が既に lease 切れ claimed を終端化)。詳細 = spec §3.1 追補 / `docs/superpowers/sessions/2026-08-02-ocr-2-4a-cutover-review.md` §8。

- **②-4a-cutover 完了後 follow-up(2026-08-02 smoke で判明・OT 記録指示)**:
  - **[UX・要 OT 判断] 長時間 upload で result page へ自動遷移しない**。**発動条件 = 処理が client-await のタイムアウト/応答喪失を跨ぐ長さ(5 枚で 1-2 分・6 回目 smoke で初発)**。完了遷移は `upload-form.tsx` の publish `'published'` → `router.push`(L762)+ catch の abandon-completed→遷移の 2 経路のみ。**publish の await が hang(応答喪失で resolve も throw もしない)すると両経路とも不発 → スピナー継続**(client server-action に timeout 無し=S1.9.3 で意図的廃止)。90 秒 longRunning banner で「試験一覧で確認」に誘導される(実機でこれが機能)が auto-nav せず。**mis-abandon なし**(成功は router.push で abandon 非経由・op は completed)。**修正候補**: (a)**poll ベース auto-nav**=upload page が自分の sourceDocumentId の source_document status を poll し completed で `router.push(result)`(応答喪失に堅牢・`/api/exams/status` 基盤流用可)(b)publish await に client timeout+status 確認(c)90 秒 banner に「結果を見る」link。CC 推奨=(a)。案 D の「transport lost success」の throw ケースは対処済・**hang ケースが未対処**の残余。
  - **[T14b 完了条件に追加] 新 flow の refs↔GC 整合を smoke 検証**: ① card A/B が同一 asset 参照 → A 削除 → asset が GC 対象外(refs 1 本残存)② A/B 両削除 → refs 0 → grace 経過後 GC ③ source_assets GC lane 新設(T14b 本体)。設計上 refs>0 は GC 対象外だが**新 flow の refs と GC 整合は smoke 未検証**。
  - **[②-5 文書ライブラリ設計項目] source 文書の hash dedup**（同一 PDF/画像の再 upload 時に既存 source を再利用）。source の server 実測 content-hash は保存済=土台あり。crop 画像が upload ごとに増えるのは仕様どおり(dedup は同一 operation 内 retry のみ・別 upload は fresh asset ID・OT 了解済)。
...
- **[✅ ②-4a-cutover — 実装 + 6回目 smoke 完走 = §0.5 へ集約]** upload UI を新 flow へ切替 = `19f34cb`(案 D)完了・実環境 e2e smoke 完走。詳細・残タスク(T14b/T15/T16/follow-up)は **§0.5** 参照。以下は履歴メモ:
  - ✅ upload-form の呼び出し列を新 flow へ配線済(圧縮 → `prepareUpload` → presigned PUT → `finalizeSource` → `claimOperation` → server: stage(OCR)→ crop → publish → 結果表示)。`processUpload` 呼び出し削除(`process.ts` 自体は残置=戻せる)。
  - **旧 flow 共存チェック(T4 暫定措置)の撤去** = **未実施の carry-forward**。cutover は最小配線ゆえ T4 の source_documents(processing) gate を撤去しなかった(scope 外)。**発動条件 = legacy `process.ts` 削除時**(§4 drift/撤去 trigger 参照・commit `004edf4`)。
  - ✅ end-to-end smoke 完走(happy path)。T10 #4/#5/#6(実 R2 412 冪等 / 実 sharp 決定性 / §7.3 guard)の**専用検証は 6回目 smoke では happy path 中心**ゆえ T14b smoke に GC 整合と併せて確認(§0.5 残 #2)。
  - レガシー `process.ts` の削除タイミング判断 = **launch 前 / ②-4b 合流後**(carry-forward)。
  - **②-4a S-5 からの申し送り(2026-08-05)**: 旧経路撤去(S-5b)で `upload-guard.ts` のコメントが実在しないものを 2 箇所名指しする状態になった — `:25` = 削除済 `claim-operation.ts` / `:55` = 改名済 `hasActiveProcessingUpload`(後継は `hasLiveUploadOperation`・読む表も `source_documents` → `upload_operations` に変わった)。`process.ts` / `upload-guard.ts` は revert 保険として S-5 の射程外に置いたため**意図的に未修正**。この 2 行は本 file 群を削除する task で一緒に消えるので、単独では直さない。
  - 判断記録=`docs/superpowers/sessions/2026-08-01-ocr-2-4a-t12-checkpoint.md` §8 + cutover review `2026-08-02-ocr-2-4a-cutover-review.md`。
