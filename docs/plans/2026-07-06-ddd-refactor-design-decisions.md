...
| P1 | domain 純粋層の抽出・移設 + 二重実装の仕分け・単一 source 化 | spec 起草中 | — | 現 HEAD `a11afca` grounding 済(carve-out 2 + 型 relocate 1 + 逆依存解消 1 + dedup 2)。spec = docs/superpowers/specs/2026-07-07-ddd-p1-domain-extraction-design.md |
